...
### Interview Synthesis: 
From the interviews, it is clear that parts of the visualization were not clear. From both sources, it was evident that my visualization did not reflect the total energy use by combining all of the energy sources used in each year. From the second interview, the suggestion to compare oil, gas, and coal use made me think about creating two charts instead of just one because the renewable energy use is difficult to see on the combined line chart. Using the feedback from the interviews, I realized that there may be too much data presented in the original visualization, and so I needed to decide what was relevant to the audience, which was the same audience that my interviewees thought. The interviews were helpful and I was able to gain inspiration for my final redesign. 

## Step five: build the solution
This is the final redesign that I created. Similar to the image in Step One, I decided to add other definitions that may be confusing to the audience. Because I could not use Tableau to create the hyperlink-like definitions like the original visualization that can be seen using the [link](https://ourworldindata.org/grapher/global-energy-substitution?time=1800..2023), I placed these definitions at the top as a separate appendix to reference when looking at the chart. The line chart fits best given the amount of time provided, however I thought about creating individual pie charts for each year to show the relative amount of energy from each source. From the interviews, I realized that having the total yearly energy use and the energy use per source was too much to put into one chart without confusion, so the last decision I made before finalizing the redesign was to only show the yearly energy use by source, and excluding the total yearly usage.

The final redesign that I created ended up being two separate line charts, one with the yearly energy use of non-renewable sources from 1950-2023 and the second with the yearly energy use of clean sources from 1950-2023. I define clean energy in Appendix I, which I created and placed before the visualizations so viewers would read this before jumping to the visualizations. These charts show the yearly energy of these sources alone, and I decided to not include the yearly total because I felt that I could not clearly represent them boht in one chart with Tableau. I also decided not to include the traditional and modern biomass as traditional was associated mostly with years that I decided to not include, and modern biomass is extremely small. Given that I would have had to define both of these, which could lead to confusion, I decided to omit both of them.
